"""
Project Title: AI Safety Chat - Enhanced Version
====================================================
...
def classify_with_gemini(client: genai.Client, text: str) -> Classification:
    try:
        ...
        st.warning(f"⚠️ Classifier error: {str(e)}. Using fallback scoring.")
...
def ui_header():
    st.set_page_config(
        page_title="AI Safety Chat - Enhanced", 
        page_icon="🛡️", 
        layout="wide",
        initial_sidebar_state="expanded"
    )
    st.title("🛡️ AI Safety Chat - Enhanced Edition")
...
def crisis_banner():
    st.warning(
        ...
        "Emergency services, AASRA (India): 91-22-27546669, Kiran: 1800-599-0019, "
        "US: 988, UK: 116 123"
    )
...
def main():
    ...
    with col1:
        with st.container(border=True):
            ...
            with btn_col1:
                send = st.button("🚀 Send & Analyze", use_container_width=True, type="primary")
            with btn_col2:
                clear_chat = st.button("🗑️ Clear", use_container_width=True)
            with btn_col3:
                toggle_history = st.button(
                    f"{'👁️ Hide' if st.session_state.show_history else '👁️ Show'} History", 
                    use_container_width=True
                )
            with btn_col4:
    ...
    if st.session_state.show_history and st.session_state.conversation_history:
        ...
        st.info("👁️ History is hidden. Click 'Show History' to view past messages.")
